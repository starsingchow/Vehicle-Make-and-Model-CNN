...
def data_augmenttation(dir, car_data):
    ...
    with tf.Session() as sess:
        for key, image in images.items():
            ...
            for result in results:
                ratio_images = RandomRatio(result, 2)
            ...
            print('finsh crop')
            random.seed(12345)
            EdgeEnhance_number = random.sample(range(len(cropped_images)), 15)
            save_number = random.sample(range(len(cropped_images)), 15)
            i = 0
            for cropped_image in cropped_images:
                if i not in save_number:
                    i +=1
                    continue
                if i in EdgeEnhance_number:
                    cropped_image = EdgeEnhance(cropped_image)

                cropped_image = cv2.cvtColor(cropped_image, cv2.COLOR_RGB2BGR)
                save_name = dir + '/{0}_'.format(i) + key
                print(save_name)
...
def Squash(image):
    ...
    resized = tf.image.resize_images(image, [x, y], method = 0)
    return resized.eval()
...
if __name__ == '__main__':
    ...
    data_augmenttation(DIR, car_data)
